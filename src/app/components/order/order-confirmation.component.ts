    ...
    <div class="min-h-screen bg-black flex flex-col items-center justify-center p-6">
      <div class="text-center max-w-md mx-auto">
          ...
          <div class="relative">
            ...
            <div class="w-24 h-24 mx-auto mb-6 relative">
              <svg class="w-24 h-24 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2" class="animate-pulse"/>
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4" class="animate-bounce"/>
              </svg>
            </div>
          </div>
        </div>
        ...
        <div *ngIf="order()" class="bg-gray-900 rounded-2xl p-6 mb-8 text-left">
          ...
          <div class="space-y-3 text-sm">
            ...
            <div class="flex justify-between">
              ...
              <span class="text-green-400 font-semibold">{{ getStatusText() }}</span>
            </div>
            ...
            <div class="flex justify-between">
              ...
              <span class="text-white">{{ order()!.paymentMethod === 'cash' ? 'Efectivo' : 'Online' }}</span>
            </div>
            
            <div class="border-t border-gray-700 pt-3 mt-3">
              <span class="text-gray-400 block mb-1">Dirección de Entrega:</span>
        ...
        <div class="bg-yellow-900 bg-opacity-50 rounded-xl p-4 mb-8">
          <p class="text-yellow-200 text-sm mb-1">Tiempo estimado de entrega:</p>
      ...
      <div class="fixed inset-0 -z-10 overflow-hidden">
        <div class="absolute top-20 left-20 w-32 h-32 bg-green-500 opacity-10 rounded-full animate-pulse"></div>
        <div class="absolute bottom-32 right-32 w-24 h-24 bg-yellow-500 opacity-10 rounded-full animate-bounce"></div>
        <div class="absolute top-1/2 left-1/4 w-16 h-16 bg-red-500 opacity-10 rounded-full animate-ping"></div>
      </div>
    </div>
  `
})
