      ...
      <div class="p-4 border-2 border-orange-200 rounded-lg bg-orange-50">
        ...
        <div class="mt-3 flex items-center gap-3">
          ...
          <button
            type="button"
            (click)="regenerate()"
            class="px-3 py-2 text-xs font-semibold bg-white border-2 border-orange-200 rounded-lg hover:border-red-500 text-red-700"
          >
            Nueva imagen
          </button>
        </div>
        ...
        <div class="mt-3">
          <label class="block text-sm text-red-700 mb-1"
            >Ingresa el texto mostrado</label
          >
          ...
          <p *ngIf="error()" class="text-xs text-red-600 mt-2">
            Texto incorrecto, vuelve a intentarlo
          </p>
          <p *ngIf="isVerified()" class="text-xs text-green-700 mt-2">
...
export class HumanVerificationComponent implements AfterViewInit {
  ...
  check(): void {
    ...
    const ok =
      ...
      normalizedInput === this.targetText().toUpperCase();
