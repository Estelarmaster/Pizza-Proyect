    ...
    <header class="bg-amber-900 text-white shadow-lg">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-16">
          ...
          <div class="flex items-center space-x-4">
            <!-- Cart -->
            <a routerLink="/cart" class="relative p-2 hover:bg-amber-800 rounded-md">
              <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="relative" *ngIf="authService.isAuthenticated()">
              <button 
                ...
                <ng-template #defaultAvatar>
                  <div class="w-8 h-8 bg-amber-600 rounded-full flex items-center justify-center">
                    <span class="text-sm font-semibold">{{ authService.user()?.name?.charAt(0) }}</span>
                  </div>
                </ng-template>
              </button>
              ...
              <div *ngIf="userMenuOpen()" 
                ...
                <div *ngIf="authService.isAdmin()" class="border-t border-gray-100">
                  <a routerLink="/admin" 
                     (click)="userMenuOpen.set(false)"
                     class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                    Dashboard Admin
        ...
        <!-- Mobile menu -->
        <div *ngIf="mobileMenuOpen()" class="md:hidden border-t border-amber-800 pt-4 pb-4">
          <div class="space-y-2">
            <a routerLink="/menu" 
