    ...
    <div class="min-h-screen bg-gray-100">
      ...
      <div class="bg-white shadow-sm border-b">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="flex justify-between items-center h-16">
            ...
            <div class="flex items-center space-x-4">
              <span class="text-gray-600">¡Hola, {{ authService.user()?.name }}!</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div *ngIf="activeTab() === 'pizzas'">
          ...
          <!-- Add/Edit Pizza Form -->
          <div *ngIf="showAddForm() || editingPizza()" class="bg-white rounded-lg shadow-md p-6 mb-6">
            <h3 class="text-lg font-bold mb-4">
              {{ editingPizza() ? 'Editar Pizza' : 'Agregar Nueva Pizza' }}
            </h3>
            
            <form (ngSubmit)="savePizza()" #pizzaForm="ngForm" class="space-y-4">
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Ingredientes (separados por coma)</label>
                <input
                  type="text"
                  name="ingredients"
                ...
                <tr *ngFor="let pizza of pizzaService.getPizzas()()">
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="flex items-center">
                      <img [src]="pizza.image" [alt]="pizza.name" class="w-12 h-12 rounded-lg object-cover mr-4">
                      <div>
                        <div class="text-sm font-medium text-gray-900">{{ pizza.name }}</div>
                        <div class="text-sm text-gray-500">{{ pizza.weight }}</div>
                      </div>
                    </div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">₹{{ pizza.price }}</td>
                  <td class="px-6 py-4 whitespace-nowrap">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full"
                          [class]="pizza.available ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'">
        ...
        <div *ngIf="activeTab() === 'orders'">
          <div class="bg-white rounded-lg shadow-md overflow-hidden">
            ...
            <div class="divide-y divide-gray-200">
              <div *ngFor="let order of orderService.getAllOrders()" 
                   class="p-6 hover:bg-gray-50 transition-colors">
                <div class="flex items-center justify-between">
                  <div>
                    <p class="font-medium text-gray-900">#{{ order.id }}</p>
                    <p class="text-sm text-gray-500">{{ order.createdAt | date:'short' }}</p>
                    <p class="text-sm text-gray-500">{{ order.items.length }} items - ₹{{ order.total }}</p>
                  </div>
                  <div class="flex items-center space-x-4">
        ...
        <div *ngIf="activeTab() === 'stats'">
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="bg-white rounded-lg shadow-md p-6">
              <h3 class="text-lg font-medium text-gray-900 mb-2">Total de Pizzas</h3>
              <p class="text-3xl font-bold text-red-600">{{ pizzaService.getPizzas()().length }}</p>
            </div>
            
            <div class="bg-white rounded-lg shadow-md p-6">
              <h3 class="text-lg font-medium text-gray-900 mb-2">Órdenes Totales</h3>
              <p class="text-3xl font-bold text-green-600">{{ orderService.getAllOrders().length }}</p>
            </div>
            
            <div class="bg-white rounded-lg shadow-md p-6">
              <h3 class="text-lg font-medium text-gray-900 mb-2">Ingresos Totales</h3>
              <p class="text-3xl font-bold text-blue-600">₹{{ getTotalRevenue() }}</p>
            </div>
          </div>
        </div>
      </div>
...
export class AdminDashboardComponent {
  ...
  getTotalRevenue(): number {
    return this.orderService.getAllOrders().reduce((total, order) => total + order.total, 0);
  }
}
