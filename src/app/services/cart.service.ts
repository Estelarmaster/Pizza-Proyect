...
export class CartService {
  ...
  readonly subtotal = computed(() => this.items().reduce((total, item) => total + (item.pizza.price * item.quantity), 0));
  ...
  readonly total = computed(() => this.subtotal() + this.taxes() + this.deliveryFee());

  addToCart(pizza: Pizza, size: 'S' | 'M' | 'L' = 'M', quantity: number = 1): void {
    const existingItemIndex = this.items().findIndex(
      item => item.pizza.id === pizza.id && item.size === size
    );

    if (existingItemIndex >= 0) {
      this.updateQuantity(existingItemIndex, this.items()[existingItemIndex].quantity + quantity);
    } else {
