...
export class PaypalService {
  ...
  loadSdk(currency: string = 'USD'): Promise<void> {
    ...
    if (this.scriptLoaded && this.currentCurrency === currency) return Promise.resolve();
    // If loaded but currency differs, remove existing SDK and reload
    if (this.scriptLoaded && this.currentCurrency !== currency) {
      const existing = document.querySelector('script[src*="paypal.com/sdk/js"]') as HTMLScriptElement | null;
